describe('pages/clientDB/permission-table-compound/permission-table-compound.vue', () => {
	...
	it('创建--未登陆', async () => {
		...
		await page.setData({'currentRole':0,'typeIndex':0})
		const createUnloginIndex = await page.data('typeIndex')
		const createUnloginRole = await page.data('currentRole')
		console.log("创建--未登陆: ",createUnloginIndex,createUnloginRole,
									createUnloginIndex === 0 ,createUnloginRole === 0);
		
		
		if(createUnloginIndex === 0 && createUnloginRole === 0 ){
	...
	it('读取--未登陆', async () => {
		...
		await page.setData({'currentRole':0,'typeIndex':1})
		const readUnloginIndex = await page.data('typeIndex')
		const readUnloginRole = await page.data('currentRole')
		console.log("读取--未登陆: ",readUnloginIndex,readUnloginRole,
									readUnloginIndex === 1 ,readUnloginRole === 0);
		
		
		if(readUnloginIndex === 1 && readUnloginRole === 0 ){
	...
	it('更新--未登陆', async () => {
		...
		await page.setData({'currentRole':0,'typeIndex':2})
		const updateUnloginIndex = await page.data('typeIndex')
		const updateUnloginRole = await page.data('currentRole')
		console.log("更新--未登陆: ",updateUnloginIndex,updateUnloginRole,
									updateUnloginIndex === 2 ,updateUnloginRole === 0);
		
		
		if(updateUnloginIndex === 2 && updateUnloginRole === 0 ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('删除--未登陆', async () => {
		...
		await page.setData({'currentRole':0,'typeIndex':3})
		const deleteUnloginIndex = await page.data('typeIndex')
		const deleteUnloginRole = await page.data('currentRole')
		console.log("删除--未登陆: ",deleteUnloginIndex,deleteUnloginRole,
									deleteUnloginIndex === 3 ,deleteUnloginRole === 0);
		
		if(deleteUnloginIndex === 3 && deleteUnloginRole === 0 ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('创建--用户', async () => {
		...
		await page.setData({'currentRole':'user','typeIndex':0})
		const createUserIndex = await page.data('typeIndex')
		const createUserRole = await page.data('currentRole')
		console.log("创建--用户: ",createUserIndex,createUserRole,
									createUserIndex === 0 ,createUserRole === 'user');
		
		
		if(createUserIndex === 0 && createUserRole === 'user' ){
	...
	it('读取--用户', async () => {
		...
		await page.setData({'currentRole':'user','typeIndex':1})
		const readUserIndex = await page.data('typeIndex')
		const readUserRole = await page.data('currentRole')
		console.log("读取--用户: ",readUserIndex,readUserRole,
									readUserIndex === 1 ,readUserRole === 'user');
		
		
		if(readUserIndex === 1 && readUserRole === 'user'){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('更新--用户', async () => {
		...
		await page.setData({'currentRole':'user','typeIndex':2})
		const updateUserIndex = await page.data('typeIndex')
		const updateUserRole = await page.data('currentRole')
		console.log("更新--用户: ",updateUserIndex,updateUserRole,
									updateUserIndex === 2 ,updateUserRole === 'user');
		
		if(updateUserIndex === 2 && updateUserRole === 'user'){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('删除--用户', async () => {
		...
		await page.setData({'currentRole':'user','typeIndex':3})
		const deleteUserIndex = await page.data('typeIndex')
		const deleteUserRole = await page.data('currentRole')
		console.log("更新--用户: ",deleteUserIndex,deleteUserRole,
									deleteUserIndex === 3 ,deleteUserRole === 'user');
		
		if(deleteUserIndex === 3 && deleteUserRole === 'user' ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('创建--审核员', async () => {
		...
		await page.setData({'currentRole':'auditor','typeIndex':0})
		const createAuditorIndex = await page.data('typeIndex')
		const createAuditorRole = await page.data('currentRole')
		console.log("创建--审核员: ",createAuditorIndex,createAuditorRole,
									createAuditorIndex === 0 ,createAuditorRole === 'auditor');
		
		if(createAuditorIndex === 0 && createAuditorRole === 'auditor' ){
			await page.callMethod('myFn',{
				"type":"create","index":1
			})
	...
	it('读取--审核员', async () => {
		...
		await page.setData({'currentRole':'auditor','typeIndex':1})
		const readAuditorIndex = await page.data('typeIndex')
		const readAuditorRole = await page.data('currentRole')
		console.log("读取--审核员: ",readAuditorIndex,readAuditorRole,
									readAuditorIndex === 1 ,readAuditorRole === 'auditor');
		
		if(readAuditorIndex === 1 && readAuditorRole === 'auditor' ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('更新--审核员', async () => {
		...
		await page.setData({'currentRole':'auditor','typeIndex':2})
		const updateAuditorIndex = await page.data('typeIndex')
		const updateAuditorRole = await page.data('currentRole')
		console.log("更新--审核员: ",updateAuditorIndex,updateAuditorRole,
									updateAuditorIndex === 2,updateAuditorRole === 'auditor');
		
		if(updateAuditorIndex === 2 && updateAuditorRole === 'auditor'){
	...
	it('删除--审核员', async () => {
		...
		await page.setData({'currentRole':'auditor','typeIndex':3})
		const deleteAuditorIndex = await page.data('typeIndex')
		const deleteAuditorRole = await page.data('currentRole')
		console.log("删除--审核员: ",deleteAuditorIndex,deleteAuditorRole,
									deleteAuditorIndex === 3,deleteAuditorRole === 'auditor');
		
		if(deleteAuditorIndex === 3 && deleteAuditorRole === 'auditor'){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('创建--管理员', async () => {
		...
		await page.setData({'currentRole':'admin','typeIndex':0})
		const createAdminIndex = await page.data('typeIndex')
		const createAdminRole = await page.data('currentRole')
		console.log("创建--管理员: ",createAdminIndex,createAdminRole,
									createAdminIndex === 0,createAdminRole === 'admin');
		
		
		if(createAdminIndex === 0 && createAdminRole === 'admin'){
			await page.callMethod('myFn',{
				"type":"create","index":1
			})
	...
	it('读取--管理员', async () => {
		...
		await page.setData({'currentRole':'admin','typeIndex':1})
		const readAdminIndex = await page.data('typeIndex')
		const readAdminRole = await page.data('currentRole')
		console.log("读取--管理员: ",readAdminIndex,readAdminRole,
									readAdminIndex === 1,readAdminRole === 'admin');
		
		
		if(readAdminIndex === 1 && readAdminRole === 'admin' ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('更新--管理员', async () => {
		...
		await page.setData({'currentRole':'admin','typeIndex':2})
		const updateAdminIndex = await page.data('typeIndex')
		const updateAdminRole = await page.data('currentRole')
		console.log("读取--管理员: ",updateAdminIndex,updateAdminRole,
									updateAdminIndex === 2,updateAdminRole === 'admin');
		
		
		if(updateAdminIndex === 2 && updateAdminRole === 'admin' ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
	...
	it('删除--管理员', async () => {
		...
		await page.setData({'currentRole':'admin','typeIndex':3})
		const deleteAdminIndex = await page.data('typeIndex')
		const deleteAdminRole = await page.data('currentRole')
		console.log("读取--管理员: ",deleteAdminIndex,deleteAdminRole,
									deleteAdminIndex === 3,deleteAdminRole === 'admin');
		
		
		if(deleteAdminIndex === 3 && deleteAdminRole === 'admin' ){
			await page.callMethod('myFn',{
				"type":"create",
				"index":0
			})
