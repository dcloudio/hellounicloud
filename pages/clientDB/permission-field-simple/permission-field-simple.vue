			...
			<view class="item" v-if="item.exclude !== type">
				...
				<template v-if="[0,1,2].includes(index)">
					<button type="primary" size="mini"
						@click="myFn({type,index,field:'_id,state,create_time,text'})">{{typeText}}不含字段{{item.field}}的记录</button>
				</template>
			</view>
		</template>
				...
				try {
					...
					if (!this.isTestMode) {
						uni.showModal({
							...
							content: item.explain + '【' + this.typeText + '字段' + item.field + '】' + (item.explain_end ? item
								.explain_end : ''),
							showCancel: false
						});
					}
