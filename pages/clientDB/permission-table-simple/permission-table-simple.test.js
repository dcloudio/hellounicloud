describe('权限表测试', () => {
  ...
  describe('创建操作测试', () => {
    ...
    describe('未登录用户创建测试', () => {
      ...
      it('限特定角色-创建数据', async () => {
        const res = await page.callMethod('myFn', { type: 'create', index: 5 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-创建数据', async () => {
        const res = await page.callMethod('myFn', { type: 'create', index: 6 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-创建数据(带action)', async () => {
        const res = await page.callMethod('myFn', { type: 'create', index: 6, action: 'add_view_count' });
        expect(res.id).toBeTruthy();
      });
    });
  ...
  describe('读取操作测试', () => {
    ...
    describe('未登录用户读取测试', () => {
      ...
      it('限特定角色-读取数据', async () => {
        const res = await page.callMethod('myFn', { type: 'read', index: 5 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-读取数据', async () => {
        const res = await page.callMethod('myFn', { type: 'read', index: 6 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-读取数据(带action)', async () => {
        const res = await page.callMethod('myFn', { type: 'read', index: 6, action: 'add_view_count' });
        expect(res.data.length).toBeGreaterThan(0);
      });
    });
  ...
  describe('更新操作测试', () => {
    ...
    describe('未登录用户更新测试', () => {
      ...
      it('限特定角色-更新数据', async () => {
        const res = await page.callMethod('myFn', { type: 'update', index: 5 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-更新数据', async () => {
        const res = await page.callMethod('myFn', { type: 'update', index: 6 });
        expect(res.errMsg).toContain('未能获取当前用户信息');
      });

      it('读取必须带上action-更新数据(带action)', async () => {
        const res = await page.callMethod('myFn', { type: 'update', index: 6, action: 'add_view_count' });
        expect(res.updated).toBeGreaterThan(0);
      });
    });
